...
class ProductRowData {
  ...
  // amount ke liye dedicated controller
...
class _ReceiveProductViewState extends State<ReceiveProductView> {
  ...
  Future<void> _loadSavedData() async {
    try {
      ...
      print('Error loading saved receive data: $e');
  ...
  void _checkAndAddRow<T>(RxList<T> rows, T Function() factory, String selectedItem) {
    if (rows.isNotEmpty) {
      ...
      if (rows is RxList<ProductRowData>) {
        lastHasItem = (rows.last as ProductRowData).selectedItem.isNotEmpty;
      } else if (rows is RxList<PackageRowData>) {
        lastHasItem = (rows.last as PackageRowData).selectedItem.isNotEmpty;
      }
      if (lastHasItem) rows.add(factory());
    }
  }

  // ─── Save single product row (create or update) ────────────
  ...
  Future<void> _deleteProductRow(int index) async {
    ...
    try {
      final result = await _apiController.deleteReceiveProduct(row.savedId!);
      if (result['success'] == true) {
        row.dispose();
  ...
  Future<void> _deletePackageRow(int index) async {
    ...
    try {
      final result = await _apiController.deleteReceivePackage(row.savedId!);
      if (result['success'] == true) {
        row.dispose();
  ...
  // ─── Original Save All (top Save button) ───────────────────
  Future<void> _saveAllData() async {
    ...
    try {
      ...
      if (saved == 0) _showAlert('No new data to save', isError: true);
    } finally {
