...
class _ConsumeProductViewState extends State<ConsumeProductView> {
  ...
  Future<void> _fetchAllConsumeProducts() async {
    ...
    try {
      ...
      for (final item in data) {
        ...
        row.recalculate();
        row.calculatedFinal.value = _toDouble(
          item['final'] ?? row.calculatedFinal.value,
        );
        row.calculatedCost.value = _toDouble(
          item['cost'] ?? row.calculatedCost.value,
        );
        row.calculatedVolume.value = _toDouble(
          item['volumeBbl'] ?? row.calculatedVolume.value,
        );
        productRows.add(row);
  ...
  void _rebalanceDistributeVolumes() {
    ...
    if (distributeRows[0].pit.isNotEmpty) {
      ...
      final formattedVol = remaining >= 0
    ...
    try {
      ...
      if (row.savedId == null) {
        ...
        if (result['success'] == true) {
          row.recalculate();
          row.calculatedFinal.value = _toDouble(
            result['data']?['final'] ?? row.calculatedFinal.value,
          );
          row.calculatedCost.value = _toDouble(
            result['data']?['cost'] ?? row.calculatedCost.value,
          );
          row.calculatedVolume.value = _toDouble(
            result['data']?['volumeBbl'] ?? row.calculatedVolume.value,
          );
      ...
      DataCell(
        Obx(() {
          ...
          if (isFinalMode) {
            return _editField(
              key: ValueKey(
                'final_edit_${row.savedId ?? i}_${row.productName}',
              ),
              value: row.used,
              width: 75,
              locked: locked,
  ...
  Widget _buildDistributePitDropdown(
    ...
    final validValues = {
      kEmpty,
      kActiveSystem,
      ...unselectedPits.map((p) => p.pitName),
    };
...
class ProductRowData {
  ...
  void recalculate() {
    ...
    calculatedVolume.value = _calculateConsumeProductVolumeBbl(
      used: uVal,
      sg: sVal,
      unit: unit,
    );
...
double _calculateConsumeProductVolumeBbl({
  ...
  final parsedUnit = _parseConsumeProductUnit(unit);
  final totalUnits = used * parsedUnit.amount;
  ...
  if (unitClass.contains('gal')) {
    ...
    volumeBbl = sg > 0 ? (totalUnits * 2000) / (sg * 350) : 0.0;
      ...
      unitClass == 'litre' ||
      unitClass == 'litres') {
    volumeBbl = totalUnits / 158.987;
  } else if (unitClass == 'ml') {
    volumeBbl = totalUnits / 158987;
  } else if (unitClass == 'm3' || unitClass == 'm^3') {
    volumeBbl = totalUnits * 6.28981;
  } else {
    volumeBbl = sg > 0 ? totalUnits / (sg * 158.987) : 0.0;
  }

  return double.parse(volumeBbl.toStringAsFixed(3));
}

({double amount, String unitClass}) _parseConsumeProductUnit(String unit) {
  final rawValue = unit.trim();
  final match = RegExp(
    r'^([0-9]*\.?[0-9]+)\s*([a-zA-Z]+)',
  ).firstMatch(rawValue);

  final amount = double.tryParse(match?.group(1) ?? '') ?? 1.0;
  final unitClass = (match?.group(2) ?? rawValue).trim().toLowerCase();

  return (amount: amount > 0 ? amount : 1.0, unitClass: unitClass);
}
