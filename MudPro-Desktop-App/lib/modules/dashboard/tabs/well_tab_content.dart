...
String _displayUnitPlain(String paramNumber, String fallback) {
  if (fallback.isEmpty) {
    final unit = AppUnits.displayUnit(paramNumber, fallback: '-');
    return unit == '-' ? '' : AppUnits.stripBrackets(unit);
  }
  final normalizedFallback =
      fallback.startsWith('(') ? fallback : '($fallback)';
  return AppUnits.stripBrackets(
    AppUnits.displayUnit(paramNumber, fallback: normalizedFallback),
  );
}

String _displayUnitWrapped(String paramNumber, String fallback) {
  final normalizedFallback =
      fallback.isEmpty ? '-' : (fallback.startsWith('(') ? fallback : '($fallback)');
  return AppUnits.displayUnit(paramNumber, fallback: normalizedFallback);
}
...
class LeftPortion extends StatelessWidget {
  const LeftPortion({super.key});

  @override
  Widget build(BuildContext context) => const GeneralSection();
}

// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
//  GENERAL SECTION  â€” UNCHANGED
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
class _GeneralSectionState extends State<GeneralSection> {
  ...
  @override
  void dispose() {
    for (final controller in fc.values) {
      controller.dispose();
    }
    super.dispose();
  }
          ...
          child: SingleChildScrollView(
            child: Table(
              ...
              children: [
                ...
                _tfRow("MD", "MD", _displayUnitPlain('1', 'ft')),
                _tfRow("TVD", "TVD", _displayUnitPlain('1', 'ft')),
                _tfRow("Inc", "Inc", "Â°"),
                _tfRow("Azi", "Azi", "Â°"),
                _tfRow("WOB", "WOB", _displayUnitPlain('20', 'lbf')),
                _tfRow("Rot. Wt.", "Rot. Wt.", _displayUnitPlain('20', 'lbf')),
                _tfRow("S/O Wt.", "S/O Wt.", _displayUnitPlain('20', 'lbf')),
                _tfRow("P/U Wt.", "P/U Wt.", _displayUnitPlain('20', 'lbf')),
                _tfRow("RPM", "RPM", "rpm"),
                _tfRow("ROP", "ROP", _displayUnitPlain('15', 'ft/hr')),
                _tfRow("Off-bottom TQ", "Off-bottom TQ", _displayUnitPlain('21', 'ft-lb')),
                _tfRow("On-bottom TQ", "On-bottom TQ", _displayUnitPlain('21', 'ft-lb')),
                _tfRow("Suction T.", "Suction T.", "Â°F"),
                _tfRow("Bottom T.", "Bottom T.", "Â°F"),
                _ddRow("Interval", selectedInterval, intervalOptions, (v) {
                  setState(() => selectedInterval = v!);
                  _sync();
                }),
                _tfRow("FIT", "FIT", _displayUnitPlain('33', 'ppg')),
                _tfRow("Formation", "Formation", ""),
                _tfRow("Additional Footage", "Additional Footage", _displayUnitPlain('1', 'ft')),
                _tfRow("NPT Time", "NPT Time", "hr"),
                _tfRow("NPT Cost", "NPT Cost", "\$"),
                _tfRow("Depth Drilled", "Depth Drilled", _displayUnitPlain('1', 'ft')),
              ],
            ),
          ),
        ),
...
class MiddlePortion extends StatefulWidget {
  ...
  @override
  State<MiddlePortion> createState() => _MiddlePortionState();
}
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (ctx, bc) {
        ...
        final double fallbackHeight = MediaQuery.sizeOf(context).height * 0.72;
        final double boundedHeight = bc.maxHeight.isFinite
            ? bc.maxHeight
            : fallbackHeight;
        final double totalH = math.max(320.0, boundedHeight - bottomPad);
        final double flexH = math.max(0.0, totalH - cementRowH - gap * 3);
  ...
  Widget _cementRow() => SingleChildScrollView(
    ...
    child: Row(
      children: [
        ...
        Text(
          "Cement Plug Vol. ${_displayUnitWrapped('6', 'bbl')}",
          style: const TextStyle(fontSize: 10),
        ),
        const SizedBox(width: 6),
        ...
        Text(
          "Plug Top ${_displayUnitWrapped('1', 'ft')}",
          style: const TextStyle(fontSize: 10),
        ),
        const SizedBox(width: 6),
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
//  CASED HOLE  â† ONLY THIS SECTION IS CHANGED
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
class CasedHoleSection extends StatefulWidget {
  const CasedHoleSection({super.key});

  @override
  State<CasedHoleSection> createState() => _CasedHoleSectionState();
}

class _CasedHoleSectionState extends State<CasedHoleSection> {
  final c = Get.find<DashboardController>();

  // â”€â”€ Casing controller to fetch from API â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  void initState() {
    super.initState();
    // Get or put the UgStController â€” it also fetches casings on init
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // â”€â”€ HEADER (only the dropdown source changed) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        SizedBox(
          height: _kHeaderH,
          child: Row(
            children: [
              ...
              // â”€â”€ Dynamic dropdown from API â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
              Obx(() {
              ...
              // â”€â”€ Add button â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
              InkWell(
                onTap: _addCasingRow,
                child: Icon(
                  Icons.add_box,
        ...
        // â”€â”€ TABLE (completely unchanged) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        Expanded(
          child: LayoutBuilder(
            builder: (ctx, bc) {
                    ...
                    children: [
                      TableRow(
                        ...
                        children: [
                          'No.',
                          'Description',
                          'OD\n(${_displayUnitPlain('2', 'in')})',
                          'Wt.\n(${_displayUnitPlain('31', 'lb/ft')})',
                          'ID\n(${_displayUnitPlain('2', 'in')})',
                          'Top\n(${_displayUnitPlain('1', 'ft')})',
                          'Shoe\n(${_displayUnitPlain('1', 'ft')})',
                          'Len.\n(${_displayUnitPlain('1', 'ft')})',
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
//  OPEN HOLE â€” UNCHANGED
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
class OpenHoleSection extends StatefulWidget {
  const OpenHoleSection({super.key});

  @override
  State<OpenHoleSection> createState() => _OpenHoleSectionState();
}
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
//  DRILL STRING â€” UNCHANGED
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
class DrillStringSection extends StatefulWidget {
  const DrillStringSection({super.key});

  @override
  State<DrillStringSection> createState() => _DrillStringSectionState();
}
                  ...
                  child: Obx(
                    () => Table(
                      ...
                      children: [
                        ...
                        ...ds.entries.asMap().entries.map(
                          (e) => _dsRow(e.key, e.value),
                        ),
                      ],
                    ),
                  ),
              ...
              child: Row(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Text(
                        "Total Length ${_displayUnitWrapped('1', 'ft')}",
                        style: TextStyle(
                          fontSize: 9,
                          fontWeight: FontWeight.w600,
                        ),
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
//  BIT SECTION â€” UNCHANGED
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
class BitSection extends StatefulWidget {
  const BitSection({super.key});

  @override
  State<BitSection> createState() => _BitSectionState();
}
          ...
          child: SingleChildScrollView(
            child: Table(
              ...
              children: [
                ...
                _bRow("Size", "Size", _displayUnitWrapped('2', 'in')),
                _bRow("Depth-in", "Depth-in", _displayUnitWrapped('1', 'ft')),
                _bRow("Depth", "Depth", _displayUnitWrapped('1', 'ft')),
              ],
            ),
          ),
        ),
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
//  NOZZLE SECTION â€” UNCHANGED
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
class NozzleSection extends StatefulWidget {
  const NozzleSection({super.key});

  @override
  State<NozzleSection> createState() => _NozzleSectionState();
}
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
//  TIME DISTRIBUTION â€” UNCHANGED
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
class TimeDistributionSection extends StatefulWidget {
  final OthersController activityController;
  const TimeDistributionSection({super.key, required this.activityController});

  @override
  State<TimeDistributionSection> createState() =>
